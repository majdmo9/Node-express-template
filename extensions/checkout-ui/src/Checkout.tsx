import { reactExtension, Banner, BlockStack, Checkbox, useApi, Pressable, Button } from "@shopify/ui-extensions-react/checkout";
...
function Extension() {
  ...
  const handleSave = async () => {
    try {
      ...
      const productsList = Object.entries(products)
        .map(([key, value]) => (value.checked ? { id: key.split("/")[key.split("/").length - 1], quantity: value.quantity } : ""))
        .filter(Boolean);
      ...
      await fetch(`${extension.scriptUrl.split("/extensions")[0]}/api/save-cart`, {
        method: "POST",
        body: JSON.stringify({ products: productsList, customer_id: customerId }),
        headers: {
  ...
  useEffect(() => {
    ...
    setCustomerId(customer?.current?.id);
  }, []);
  ...
  return (
    <BlockStack border={"base"} borderRadius="base" padding={"tight"}>
      ...
      <Button loading={loading} onPress={handleSave} disabled={Object.values(products).every(el => !el.checked) || loading || saved}>
